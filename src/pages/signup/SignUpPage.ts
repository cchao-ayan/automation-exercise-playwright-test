...
export class SignUpPage extends BasePage {
  ...
  async fillSignUpForm() {
    ...
    await this.expectEquals(
      locators.nameInput(this.page),
      testData.signUp.name,
    ); // Verify name input is pre-filled with the name used during signup
    await this.expectEquals(
      locators.emailInput(this.page),
      testData.signUp.email,
    ); // Verify email input is pre-filled with the email used during signup
    await this.fill(
      locators.passwordInput(this.page),
      testData.signUp.password,
    );
    await this.selectByValue(
      locators.dateOfBirthDay(this.page),
      testData.signUp.day,
    );
    await this.selectByValue(
      locators.dateOfBirthMonth(this.page),
      testData.signUp.month,
    );
    await this.selectByValue(
      locators.dateOfBirthYear(this.page),
      testData.signUp.year,
    );
    await this.fill(
      locators.firstNameInput(this.page),
      testData.signUp.firstName,
    );
    await this.fill(
      locators.lastNameInput(this.page),
      testData.signUp.lastName,
    );
    await this.fill(locators.companyInput(this.page), testData.signUp.company);
    await this.fill(
      locators.address1Input(this.page),
      testData.signUp.address1,
    );
    await this.fill(
      locators.address2Input(this.page),
      testData.signUp.address2,
    );
    await this.selectByValue(
      locators.countrySelect(this.page),
      testData.signUp.country,
    );
    ...
    await this.fill(
      locators.mobileNumberInput(this.page),
      testData.signUp.mobileNumber,
    );
  }
