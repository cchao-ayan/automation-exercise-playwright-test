...
test.describe("Automation Exercises - Test Cases", () => {
  ...
  test("Test Case 3: Login User with incorrect email and password", async ({
    ...
    await test.step("3. Login invalid credentials", async () => {
      ...
      await pom.loginPage.ready();
      await pom.signUpPage.login(
        invalidCredentials.user1.email,
        invalidCredentials.user1.password,
      );
    });
  ...
  test("Test Case 8: Verify All Products and product detail page", async ({
    ...
    await test.step("5. Verify feature details", async () => {
      const name = await pom.productsPage.getNameInnerText();
      console.log(`Featured Item Name: ${name}`);
